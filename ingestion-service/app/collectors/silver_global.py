...
QUERY = """
query SilverHistory($start: Int!, $end: Int!) {
  silverHistory: GetMetalHistory(
    symbol: "AG"
    currency: "USD"
    startTime: $start
    endTime: $end
...
QUERY_LATEST = """
query SilverLatest($start: Int!, $end: Int!) {
  silverHistory: GetMetalHistory(
    symbol: "AG"
    currency: "USD"
    startTime: $start
...
def fetch_global_historical():
    ...
    return data["data"]["silverHistory"]["results"]
...
def insert_prices(rows):
    ...
    if cur.rowcount == 0:
        print("⚠️ No new Global Silver rows inserted (all duplicates)")
    else:
        print(f"✅ Inserted {cur.rowcount} Global Silver rows")
...
# ---------- INGEST ----------
def ingest_silver_global_historical():
...
def ingest_silver_global_latest():
